# Migration Guide: OctoBot-Backtesting
...
### 3. Source Layout
- **Layout**: `src/` layout
- **Package moved**: `octobot_backtesting/` -> `src/octobot_backtesting/`
- **Import unchanged**: `import octobot_backtesting` still works

If upstream adds files to the old location, move them to `src/octobot_backtesting/`.
...
known-first-party = ["octobot_backtesting"]
```
...
3. **Check source layout**: If upstream adds new modules to the old path, move them to `src/octobot_backtesting/`.
4. **Re-lock**: Run `uv lock` to update `uv.lock` with any new/changed dependencies.
5. **Verify**: Run `uv sync && uv run python -c "import octobot_backtesting" && uv run pytest` (if tests exist).

## Quick Commands

```bash
uv sync                                         # Install all deps
uv run python -c "import octobot_backtesting"    # Verify import
